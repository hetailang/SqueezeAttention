...
class LlamaAttention_my(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        heavy_budget_ratio = kwargs['sliding_window']
        recent_budget_ratio = kwargs['sliding_window']
        ...
        if not self.previous_scores == None:
            ...
            self.heavy_budget = int(heavy_budget_ratio * current_scores_sum.shape[-1])
            self.recent_budget = int(recent_budget_ratio * current_scores_sum.shape[-1])
...
class LlamaModel_my(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if self.hiddlayer == True and past_key_value is None:
            ...
            hidd_data = [np.mean(hidd_data[i]) for i in hidd_data]
            ...
            for i in range(len(class1_indices)):
                self.sliding_windows[class1_indices[i]] = ini_size
            for i in range(len(class2_indices)):
                self.sliding_windows[class2_indices[i]] = (1 + percent) / 2 * ini_size
            for i in range(len(class3_indices)):
                self.sliding_windows[class3_indices[i]] = ini_size * percent
...
def convert_kvpruner_llama(model, config):
    ...
    if not has_entered_function_convert:
        print('convert llama model with kvpruner...')
