...
def main():
    ...
    parser.add_argument("--model_arch", type=str, default='test')
    parser.add_argument("--model_name", type=str, default='facebook/opt-350m')
    parser.add_argument("--cache_dir", type=str, default='../../checkpoint')
    parser.add_argument("--model_path", type=str, default = '/home/user/.cache/huggingface/hub/models--facebook--opt-350m')

    parser.add_argument("--heavy_ratio", type=float, default=0.1)
    parser.add_argument("--recent_ratio", type=float, default=0.1)
    parser.add_argument("--start_layer", type=int, default=24)

    parser.add_argument("--length", type=int, default=64) # 可修改生成文本长度
    ...
    #prompt_text = 'how many hours in a day?'

    model_name = args.model_name
    model_path = args.model_path
    config = AutoConfig.from_pretrained(model_path, cache_dir=args.cache_dir)
    config.heavy_ratio = args.heavy_ratio
    config.recent_ratio = args.recent_ratio
    config.start = args.start_layer
    #print(config)
    tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=True, cache_dir=args.cache_dir)

    ######## Generate with Full Cache
    model = AutoModelForCausalLM.from_pretrained(model_path, cache_dir=args.cache_dir)
